...
void *raytracing_pthread(void *a)
{
    ...
    calculateBasisVectors(u, v, w, para->view);

    idx_stack stk;
    int factor = sqrt(SAMPLES);
    for (int j = para->height_start; j < para->height; j += THREAD_NUMBER) {
        for (int i = para->width_start; i < para->width; i ++) {
            double r = 0, g = 0, b = 0;
                ...
                int width, int height)
{
    ...
    for (int i = 0; i < THREAD_NUMBER; i++) {
        ...
        paras[i].background_color[0] = background_color[0];
        paras[i].background_color[1] = background_color[1];
        paras[i].background_color[2] = background_color[2];
        paras[i].rectangulars = rectangulars;
        paras[i].spheres = spheres;
        paras[i].lights = lights;
        paras[i].view = view;
        paras[i].width = width;
        paras[i].height = height;
        paras[i].width_start = 0;
        paras[i].height_start = i;
    ...
    int factor = sqrt(SAMPLES);
#ifdef OMP
    #pragma omp parallel for num_threads(THREAD_NUMBER) private(stk, d, object_color) schedule(static,1)
#endif
